...
class Metro(Map):
    def __init__(self) -> None:
        ...
        Scene.clear()
        S = V.ZERO
        self.UNDERPASS_ENTRANCE(S)
        self.BUS_STOP(S + V.RIGHT * (UNDERPASS_ENTRANCE_SLOPE + UNDERPASS_HALLWAY_WIDTH + UNDERPASS_ENTRANCE_STAIRS))
        self.create("Metro")
    def UNDERPASS_ENTRANCE(self, S: V = V(0, 0, 0)) -> None:
        self.UNDERPASS_SIDEWALK(S)
        S += V.BACKWARD * STREET_SIDEWALK_WIDTH
        self.UNDERPASS_SLOPE(S)
        self.UNDERPASS_HALL(S + V.RIGHT * UNDERPASS_ENTRANCE_SLOPE)
        self.UNDERPASS_STAIRS(S + V.RIGHT * (UNDERPASS_ENTRANCE_SLOPE + UNDERPASS_HALLWAY_WIDTH))
