...
STREET_BUSSTOP_LENGTH = 45
STREET_BUSSTOP_BEFORE = 15
STREET_BUSSTOP_AFTER = 10
...
class Metro(Map):
    ...
    def __init__(self):
        ...
        self.load(UnderpassSlopeEntrance, V3.BACKWARD*STREET_SIDEWALK_WIDTH+V3.RIGHT*(UNDERPASS_HALLWAY_WIDTH+UNDERPASS_STAIRS_LENGTH), (UNDERPASS_ENTRANCE_WIDTH, UNDERPASS_SLOPE_LENGTH), modifiers=[RotateRight(2)])
        ...
        self.load(StreetRoad, V3.BACKWARD*(STREET_SIDEWALK_WIDTH+STREET_LANE_WIDTH)+V3.DOWN*STREET_CURB_HEIGHT, (STREET_LANE_WIDTH*STREET_LANE_COUNT, UNDERPASS_SLOPE_LENGTH+UNDERPASS_HALLWAY_WIDTH+UNDERPASS_STAIRS_LENGTH+STREET_BUSSTOP_LENGTH))
        ...
        self.load(UnderpassSlopeEntrance, V3.BACKWARD*(STREET_SIDEWALK_WIDTH+STREET_LANE_WIDTH*(STREET_LANE_COUNT+2)-UNDERPASS_ENTRANCE_WIDTH)+V3.RIGHT*(UNDERPASS_HALLWAY_WIDTH+UNDERPASS_STAIRS_LENGTH), (UNDERPASS_ENTRANCE_WIDTH, UNDERPASS_SLOPE_LENGTH), modifiers=[RotateRight(2)])
        self.load(UnderpassEntranceSidewalk, V3.BACKWARD*(STREET_SIDEWALK_WIDTH+STREET_LANE_WIDTH*(STREET_LANE_COUNT+1)), (STREET_SIDEWALK_WIDTH+STREET_LANE_WIDTH, UNDERPASS_SLOPE_LENGTH+UNDERPASS_HALLWAY_WIDTH+UNDERPASS_STAIRS_LENGTH), modifiers=[FlipX()])
        self.load(BusStopLane, V3.RIGHT*(UNDERPASS_STAIRS_LENGTH+UNDERPASS_HALLWAY_WIDTH+UNDERPASS_SLOPE_LENGTH), (STREET_SIDEWALK_WIDTH+STREET_LANE_WIDTH, STREET_BUSSTOP_LENGTH))
        self.load(BusStopLane, V3.BACKWARD*(STREET_SIDEWALK_WIDTH+STREET_LANE_WIDTH*(STREET_LANE_COUNT+1))+V3.RIGHT*(UNDERPASS_STAIRS_LENGTH+UNDERPASS_HALLWAY_WIDTH+UNDERPASS_SLOPE_LENGTH), (STREET_SIDEWALK_WIDTH+STREET_LANE_WIDTH, STREET_BUSSTOP_LENGTH), modifiers=[RotateRight(2)])
...
def BusStopLane(self):
    """Lane & sidewalk for a bus stop
    """
    BLI, BRI = (a + STREET_CURB_WIDTH * V3.FORWARD for a in (self.BL, self.BR))
    BLI1 = self.BL + V3.RIGHT * STREET_BUSSTOP_AFTER + V3.FORWARD * STREET_LANE_WIDTH
    BRI1 = self.BR + V3.LEFT * STREET_BUSSTOP_BEFORE + V3.FORWARD * STREET_LANE_WIDTH
    BLI2, BRI2 = (a + V3.FORWARD * STREET_CURB_WIDTH for a in (BLI1, BRI1))
    self.face([self.TR, self.TL, BLI, BLI2, BRI2, BRI], TILES) ######
    self.face([self.BR, BRI, BRI2, BLI2, BLI, self.BL, BLI1, BRI1], CONCRETE) #####
    BL0, BLI0, BRI0, BR0 = (a + V3.DOWN*STREET_CURB_HEIGHT for a in (self.BL, BLI1, BRI1, self.BR))
    self.face([self.BR, BRI1, BRI0, BR0], CONCRETE) ######
    self.face([BRI1, BLI1, BLI0, BRI0], CONCRETE) ######
    self.face([BLI1, self.BL, BL0, BLI0], CONCRETE) #####
    self.face([BR0, BRI0, BLI0, BL0], ASPHALT)
